...
5. **Secret Word Guess**
   - The computer selects a random secret word, and the user must guess what it is.
   - Still implemented **without loops**, using nested `if / elif / else`.

6. **Menu Ordering System (+ Total Cost)**
  
   - A console-based menu program where the user selects an item by entering a number (1–3).
   - The program then displays the item ordered and calculates total cost.
   - An optional drink can be added before the final bill is displayed.
   
7. **High–Low Number Guessing Game**

   - The computer selects a random number between 1 and 10.  
   - The user has **three attempts** to guess the number correctly.
